...
public class PlayerMovementSystem extends IteratingSystem {
    ...
    public void setStatement(BodyComponent body, StateComponent state, PlayerComponent player)
    {
        ...
        if (isMovingKeyPressed() && !player.isAttacking)
        {
            ...
            if((body.body.getLinearVelocity().y>0 || controller.up) && state.get()!=StateComponent.MOVING_UP) state.set(StateComponent.MOVING_UP);
            if((body.body.getLinearVelocity().y<0 || controller.down) && state.get()!=StateComponent.MOVING_DOWN) state.set(StateComponent.MOVING_DOWN);
